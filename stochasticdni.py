...
parser.add_argument('-y', '--sg_inp', type=str, default='1111100',
...
def param_init_sgmod(params, prefix, units, zero_init=True):
	...
	inp_list = [14*28, 14*28, units, units, units, 1, units]
	inp_size = 0
	for i in range(7):
		if args.sg_inp[i] == '1':
			inp_size += inp_list[i]
...
if args.mode == 'train':
	...
	extra1 = T.matrix('sg_input_baseline', dtype='float32')
	extra2 = T.matrix('sg_input_complementary_probs', dtype='float32')
...
if args.mode == 'train':
	...
	while condition == False:
		...
		if iters != 0 and iters % (args.sg_epoch_rate * 600) == 0 and args.sg_learning_rate > 1e-6:
			args.sg_learning_rate /= args.sg_slash_rate
