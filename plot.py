...
legend_entries = []
for k in [1]:
	f = open('Results/disc/synthetic_gradients/tsgd_sgpre_11111_ld_1_gc0.0_100_0.0001.txt', 'r').read().splitlines()
	vals = [float(line.split(',')[3]) for line in f]
	smoothened = smooth(vals) 
	plt.plot(smoothened[::600])
	legend_entries += ['Deep Subnetworks']

	f = open('Results/disc/synthetic_gradients/tsgd_sgpre_11111_lin_1_gc0.0_100_0.0001.txt', 'r').read().splitlines()
	vals = [float(line.split(',')[3]) for line in f]
	smoothened = smooth(vals) 
	plt.plot(smoothened[::600])
	legend_entries += ['Linear Subnetworks']

	# f = open('Results/disc/SF/training_reinforce_1_100_0.0001.txt', 'r').read().splitlines()
